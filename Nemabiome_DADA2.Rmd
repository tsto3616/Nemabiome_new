...
path <- "C:/Users/tsto3616/Documents/Nemabiome_sequences"
...
#NEMA2 fwd of 35915 count and rev of 32505
...
# for (i in seq_along(fwd_files)) { system2(cutadapt, args = c(cutadapt_args, "-o", fwd_cut[i], "-p", rev_cut[i], fwd_files[i], rev_files[i]),stdout = cut_logs[i])  }
...
#including minLen to eliminate the 0 read length reads
...
write.csv(seqtab_nochim, file = "nemabiome_seqtabnochim.csv")

TSeqTab <- as.data.frame(t(seqtab_nochim))
TSeqTab$variant<-1:nrow(TSeqTab)
write.csv(TSeqTab, file = "nemabiome.csv")

Tseqtab_nochim<-read.csv("nemabiome_seqtabnochim.csv", row.names=1)
...
Taxa <- assignTaxonomy(seqtab_nochim,"C:/Users/tsto3616/Downloads/NEMA_USYD_ITS2_Database_assignTax_v3.fa",taxLevels = c("Locus", "Genus", "Species", "Subsp/Strain"), verbose = TRUE, minBoot = 50, multithread=FALSE, tryRC=TRUE) #default for minBoot is 50
print(Taxa)

write.csv(Taxa, file = "maxBoot-test.csv", row.names = TRUE)
unname(Taxa)

Taxa<-read.csv("maxBoot-test.csv")

tax.augmented <- data.frame(Taxa, t(seqtab_nochim), stringsAsFactors=FALSE)

write.csv(tax.augmented, file="ASVs_raw_masterfiles.csv", row.names=TRUE)

Species <- assignSpecies(seqtab_nochim, "C:/Users/tsto3616/OneDrive - The University of Sydney (Staff)/Desktop/Nemabiome/Usyd_as_ref.txt", tryRC=TRUE)

Species

write.csv(Species, file = "Spe-NEMA1.csv", row.names = TRUE)
unname(Species)

Species<-read_csv("Spe-NEMA1.csv")

head(Species)

seqtab<-read_csv("nemabiome_seqtabnochim.csv")

head(seqtab)

assigned_species<-full_join(Species, seqtab, by="Sequences")

head(assigned_species)

write.csv(assigned_species, file="Assigned_nemabiome_Species.csv", row.names=TRUE)
